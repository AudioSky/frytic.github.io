...
const scriptsInEvents = {
	...
	async Btns_Event104_Act1(runtime, localVars)
	{
		
		let _Levt = runtime.globalVars.Levt;
		
		let _LevtStr = _Levt.toString();
	...
	async Louder_Event3_Act1(runtime, localVars)
	{
		...
		vkBridge.send('VKWebAppStorageGet', {
		  ...
		  .then((result) => { 
		     ...
		     if(result.keys[0].value.trim().length != 0){
		
		      _CoinsGet = result.keys[0].value;
		       runtime.globalVars.Coins = parseInt(_CoinsGet);
		
		     }else if(result.keys[0].value.trim().length === 0){
		
		       runtime.globalVars.Coins = +_NumCoun;
		
		     }
		
		         //    runtime.globalVars.Levt = parseInt(result.keys[0].value);
		  ...
		  .catch((error) => {
		    ...
		    runtime.globalVars.Coins = +_NumCoun;
		    runtime.globalVars.Levt = 1;
		  });
		
		   console.log("d " + _CoinsGet);
		   console.log("s " +  runtime.globalVars.Coins);
	},
	...
	async Inventary_Event23_Act9(runtime, localVars)
	{
		
		let _Coins = runtime.globalVars.Coins;
		
		let _CoinsStr = _Coins.toString();
