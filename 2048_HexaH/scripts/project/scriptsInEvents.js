...
const scriptsInEvents = {
	...
	async Action_Event19_Act2(runtime, localVars)
	{
		vkBridge.send('VKWebAppStorageSet', {
		   key: 'education',
		   value: String(runtime.globalVars.Education)
		  })
		  .then((data) => { 
		    if (data.result) {
	...
	async Action_Event97_Act2(runtime, localVars)
	{
		vkBridge.send('VKWebAppStorageSet', {
		   key: 'diamonds',
		   value: String(runtime.globalVars.Diamonds)
		  })
		  .then((data) => { 
		    if (data.result) {
		      // Значение переменной задано
	...
	async Action_Event101_Act2(runtime, localVars)
	{
		vkBridge.send('VKWebAppStorageSet', {
		   key: 'diamonds',
		   value: String(runtime.globalVars.Diamonds)
	...
	async Action_Event207_Act2(runtime, localVars)
	{
		let St = runtime.globalVars.Score;
		
		vkBridge.send('VKWebAppStorageSet', {
		   key: 'recordsore',
		   value: JSON.stringify(runtime.globalVars.Score)
		  })
		  .then((data) => { 
		    if (data.result) {
		      // Значение переменной задано
	...
	async Buttons_Event51_Act4(runtime, localVars)
	{
		vkBridge.send('VKWebAppStorageSet', {
		   key: 'diamonds',
		   value:runtime.globalVars.Diamonds
		  })
		  .then((data) => { 
		    if (data.result) {
		      // Значение переменной задано
	...
	async Start_Event1_Act6(runtime, localVars)
	{
		...
		vkBridge.send('VKWebAppStorageGet', {
		  ...
		  console.log("dsff  " + numbers);
		  
		  	  runtime.globalVars.RecordScore = numbers.recordsore[0];
			  runtime.globalVars.Education = numbers[1];
			  runtime.globalVars.Diamonds = numbers[2];
	},
