	...
	async Start_Event1_Act6(runtime, localVars)
	{
		vkBridge.send('VKWebAppStorageGet', {
		  ...
		  .then((data) => { 
		    if (data.keys) {
		      // Значения получены
			  runtime.globalVars.RecordScore = data[recordsore[1]];
			  runtime.globalVars.Education = data[education];
			  runtime.globalVars.Diamonds = data[diamonds];
			  
			  console.log( data[recordsore[1]]);
			   console.log( data[recordsore]);
			    console.log(  runtime.globalVars.RecordScore);
		    }
		  })
		  .catch((error) => {
		    // Ошибка
