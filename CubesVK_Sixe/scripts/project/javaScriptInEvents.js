		  ...
		  .then((result) => { 
		    ...
		    try{ 
		     ...
		     for(let i = 0; i < 17; i++){
		        ...
		        if(JSON.stringify(result.keys[i].value) === '{}'){
		
		          if(i === 0){
		            runtime.globalVars.Coins = +_varEmpty[0];
		            console.log("пустой монетки " + result.keys[0].value);
		          }else if(i === 1){
		            runtime.globalVars.Levt = +_varEmpty[1];
		              console.log("пустой levt" + result.keys[1].value);
		          }else if(i === 2){
		          ...
		          if(i === 0){
		             _CoinsGet = JSON.parse(result.keys[i].value);   
		              runtime.globalVars.Coins = +_CoinsGet; 
		              console.log("непустой монетки " + result.keys[0].value);
		              console.log("непустой монетки " + _CoinsGet);
		
		          }else if(i === 1){
		             _LevtGet = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_LevtGet;  
		             console.log("пустой levt" + result.keys[i].value);
		
		          }else if(i === 2){
		             _HeroFaceGet = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_HeroFaceGet; 
		
		          }else if(i === 3){
		             _FaceVikingGet = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_FaceVikingGet; 
		
		          }else if(i === 4){
		             _FaceSoldierGet = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_FaceSoldierGet; 
		
		          }else if(i === 5){
		             _FacePirateGet = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_FacePirateGet; 
		
		          }else if(i === 6){
		             _FaceGladiator = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_FaceGladiator; 
		
		          }else if(i === 7){
		             _GunOneTwo = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunOneTwo; 
		
		          }else if(i === 8){
		             _GunOneThree = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunOneThree; 
		
		          }else if(i === 9){
		             _GunVikingTwo = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunVikingTwo; 
		
		          }else if(i === 10){
		             _GunVikingThree = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = + _GunVikingThree; 
		
		          }else if(i === 11){
		             _GunSoldierTwo = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunSoldierTwo; 
		
		          }else if(i === 12){
		             _GunSoldierThree = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunSoldierThree; 
		
		          }else if(i === 13){
		             _GunPirateTwo = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunPirateTwo; 
		
		          }else if(i === 14){
		             _GunPirateThree = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunPirateThree; 
		
		          }else if(i === 15){
		             _GunGladiatorTwo = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunGladiatorTwo; 
		
		          }else if(i === 16){
		             _GunGladiatorThree = JSON.parse(result.keys[i].value);   
		             runtime.globalVars.Coins = +_GunGladiatorThree; 
		          }//if(i === 0)
		    ...
		    //runtime.globalVars.Coins = +_CoinsGet;
		    // console.log("не пустая " + result.keys[0].value);
		    ...
		    /* //монетка
		     runtime.globalVars.Coins = +_varEmpty[0];
		     runtime.globalVars.Levt = +_varEmpty[1];
		     console.log("ошибка " + result.keys[0].value);
